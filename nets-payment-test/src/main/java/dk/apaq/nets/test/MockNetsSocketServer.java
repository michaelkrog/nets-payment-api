            ...
            public void run() {
                ...
                while (running) {
                    try {
                        ...
                        if (header.getLength() < PGTMHeader.HEADER_LENGTH) {
                            throw new ParseException("Length of data specified by header is not enough. [length=" + header.getLength() + "]", 0);
                        }

                        LOG.debug("Reading data from socket.");
                        byte[] data = readData(in, header.getLength() - PGTMHeader.HEADER_LENGTH);
                        LOG.debug("Data read.");
                        ...
                        if (running) { //Only report errors if server not marked as stopped
